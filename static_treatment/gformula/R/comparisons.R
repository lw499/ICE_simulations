...
get_plot_info <- function(outcome_name, compevent_name, time_name, time_points,
  ...
  time <- unique(obs_data[[time_name]])
  ...
  dt_cov_plot <- lapply(1:length(covnames), FUN = function(i){
    ...
    if (covtypes[i] == 'categorical'){
      sub_obs_data <- obs_data[, summary(eval(parse(text = covname))), by = time_name]
      sub_obs_data[, (covname) := levels(obs_data[[covname]])]
      sub_obs_data[, 'legend' := 'observed']
      sub_est_data <- nat_pool[, summary(eval(parse(text = covname))), by = time_name]
      sub_est_data[, (covname) := levels(nat_pool[[covname]])]
      sub_est_data[, 'legend' := 'natural course']
      ...
      obs_cov_data <- data.table(t0 = sort(time), cov = obs_results[[1]][[covname]],
                                 legend = "observed")
      est_cov_data <- data.table(t0 = 0:(time_points - 1), cov = sim_results[[1]][[covname]],
                                 legend = "natural course")
  ...
  if (outcome_type == 'survival'){
    obs_results2 <- obs_results
    if (time_points < length(time)){
      obs_results2[[2]] <- utils::head(obs_results2[[2]], time_points)
    } else {
      obs_results2[[2]] <- c(obs_results2[[2]],
                             rep(NA, length(nat_result) - length(obs_results2[[2]])))
    }

    obs_risk_data <- data.table(t0 = sort(time),
                                risk = obs_results2[[2]],
                                survival = obs_results2[[3]],
                                legend = "observed")
...
get_outgrphs <- function(x, risk, survival, xlab, ylab_risk, ylab_surv, ci_risk){
  ...
  if (x$nsamples > 0 & ci_risk){
    if (!is.na(x$interventions)[[1]][[1]]){
      risk_outgrph <-
        risk_outgrph + geom_errorbar(aes(ymin = rep(x$CI_result$V1_CI_lower, 2),
                                                       ymax = rep(x$CI_result$V1_CI_upper, 2)),
                                                   width = 0.3)
    } else {
      risk_outgrph <-
        risk_outgrph + geom_errorbar(aes(ymin = rep(x$CI_result$CI_lower, 2),
                                                       ymax = rep(x$CI_result$CI_upper, 2)),
                                                   width = 0.3)
    }
  }
