...
#' @param intvar     Vector of character strings specifying the names of variables to be intervened on in each round of the simulation.
...
static <- function(newdf, pool, intvar, intvals, time_name, t){
  set(newdf, j = intvar, value = intvals[[1]])
}
...
#' @param newdf         Data table containing the simulated data at time \eqn{t}.
#' @param pool          Data table containing the simulated data at times before \eqn{t}.
#' @param intervention  List of vectors. Each vector contains a function
...
#' @param intvar        Character string specifying the name of the variable to be intervened on in each round of the simulation.
#' @param t             Integer specifying the current time index.
intfunc <- function(newdf, pool, intervention, intvar, time_name, t){
  if (length(intervention) == 1){ # Check if intervention contains just function
    # return (intervention[[1]](newdf, pool, intvar, intvals = NULL, time_name, t))
    intervention[[1]](newdf, pool, intvar, intvals = NULL, time_name, t)
  } else {
    # If intervention contains intervention values, pass those values to intervention
    # function
    # return (intervention[[1]](newdf, pool, intvar,
    # intvals = intervention[2:length(intervention)], time_name, t))
    intervention[[1]](newdf, pool, intvar, intvals = intervention[2:length(intervention)],
                      time_name, t)
  }
}
