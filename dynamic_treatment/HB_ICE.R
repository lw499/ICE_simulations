...
afitog = glm(A ~ L1, family = binomial(), data = dffull[!is.na(dffull$lag_A) & dffull$lag_A==0,])
paramtmp = (yfitog)$coef

##################
######time 5######
##################
  y5dat = tmpdata[!is.na(tmpdata$C_4) & tmpdata$C_4==0 & tmpdata$Y_4==0,];  y5dat$A_4 = ifelse(y5dat$L1_4==0,0,1)
  y5dat$A = y5dat$A_4; y5dat$L1 = y5dat$L1_4;
  y5dat$y5pred = predict(yfitog, newdata = y5dat, type="response"); 
  
  y5fitextra = glm(y5pred ~ L1_3 + A_3 + L1_3*A_3, family = binomial(), data = y5dat) ; 
  y5dat = tmpdata[!is.na(tmpdata$C_3) & tmpdata$C_3==0  & tmpdata$Y_3==0,]; y5dat$A_3 = ifelse(y5dat$L1_3==0,0,1)
  ...
  y5fitextra = glm(y5pred ~ L1_2 + A_2 + L1_2*A_2, family = binomial(), data = y5dat) ; 
  y5dat = tmpdata[!is.na(tmpdata$C_2) & tmpdata$C_2==0  & tmpdata$Y_2==0,]; y5dat$A_2 = ifelse(y5dat$L1_2==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y5dat)), y5dat$A_2, y5dat$L1_2)  %*% matrix(paramtmp, nrow=3))
  y5dat$y5pred = predict(y5fitextra, newdata = y5dat, type="response")*(1-predicttmp) + predicttmp
  
  y5fitextra = glm(y5pred ~ L1_1 + A_1 + L1_1*A_1, family = binomial(), data = y5dat) ; 
  y5dat = tmpdata[!is.na(tmpdata$C_1) & tmpdata$C_1==0  & tmpdata$Y_1==0,]; y5dat$A_1 = ifelse(y5dat$L1_1==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y5dat)), y5dat$A_1, y5dat$L1_1)  %*% matrix(paramtmp, nrow=3))
  y5dat$y5pred = predict(y5fitextra, newdata = y5dat, type="response")*(1-predicttmp) + predicttmp
  
  y5fitextra = glm(y5pred ~ L1_0 + A_0 + L1_0*A_0, family = binomial(), data = y5dat) ;
  y5dat = tmpdata ; y5dat$A_0 = ifelse(y5dat$L1_0==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y5dat)), y5dat$A_0, y5dat$L1_0)  %*% matrix(paramtmp, nrow=3))
  y5dat$y5pred = predict(y5fitextra, newdata = y5dat, type="response")*(1-predicttmp) + predicttmp
  
  meany5 = mean(y5dat$y5pred)

##################
######time 4######
##################
  y4dat = tmpdata[!is.na(tmpdata$C_3) & tmpdata$C_3==0 & tmpdata$Y_3==0,]; y4dat$A_3 = ifelse(y4dat$L1_3==0,0,1)
  y4dat$A = y4dat$A_3; y4dat$L1 = y4dat$L1_3;
  y4dat$y4pred = predict(yfitog, newdata = y4dat, type="response"); 
  
  y4fitextra = glm(y4pred ~ L1_2 + A_2 + L1_2*A_2, family = binomial(), data = y4dat) ; 
  y4dat = tmpdata[!is.na(tmpdata$C_2) & tmpdata$C_2==0  & tmpdata$Y_2==0,]; y4dat$A_2 = ifelse(y4dat$L1_2==0,0,1)
  ...
  y4fitextra = glm(y4pred ~ L1_1 + A_1 + L1_1*A_1, family = binomial(), data = y4dat) ; 
  y4dat = tmpdata[!is.na(tmpdata$C_1) & tmpdata$C_1==0  & tmpdata$Y_1==0,]; y4dat$A_1 = ifelse(y4dat$L1_1==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y4dat)), y4dat$A_1, y4dat$L1_1)  %*% matrix(paramtmp, nrow=3))
  y4dat$y4pred = predict(y4fitextra, newdata = y4dat, type="response")*(1-predicttmp) + predicttmp
  
  y4fitextra = glm(y4pred ~ L1_0 + A_0 + L1_0*A_0, family = binomial(), data = y4dat) ; 
  y4dat = tmpdata; y4dat$A_0 = ifelse(y4dat$L1_0==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y4dat)), y4dat$A_0, y4dat$L1_0)  %*% matrix(paramtmp, nrow=3))
  y4dat$y4pred = predict(y4fitextra, newdata = y4dat, type="response")*(1-predicttmp) + predicttmp
  
  meany4 = mean(y4dat$y4pred)

##################
######time 3######
##################
  y3dat = tmpdata[!is.na(tmpdata$C_2) & tmpdata$C_2==0 & tmpdata$Y_2==0,]; y3dat$A_2 = ifelse(y3dat$L1_2==0,0,1)
  y3dat$A = y3dat$A_2; y3dat$L1 = y3dat$L1_2;
  y3dat$y3pred = predict(yfitog, newdata = y3dat, type="response"); 
  
  y3fitextra = glm(y3pred ~ L1_1 + A_1 + L1_1*A_1, family = binomial(), data = y3dat) ; 
  y3dat = tmpdata[!is.na(tmpdata$C_1) & tmpdata$C_1==0  & tmpdata$Y_1==0,]; y3dat$A_1 = ifelse(y3dat$L1_1==0,0,1)
  ...
  y3fitextra = glm(y3pred ~ L1_0 + A_0 + L1_0*A_0, family = binomial(), data = y3dat) ; 
  y3dat = tmpdata; y3dat$A_0 = ifelse(y3dat$L1_0==0,0,1)
      predicttmp = plogis(cbind(rep(1, nrow(y3dat)), y3dat$A_0, y3dat$L1_0)  %*% matrix(paramtmp, nrow=3))
  y3dat$y3pred = predict(y3fitextra, newdata = y3dat, type="response")*(1-predicttmp) + predicttmp
  
  meany3 = mean(y3dat$y3pred)

##################
######time 2######
##################
  y2dat = tmpdata[!is.na(tmpdata$C_1) & tmpdata$C_1==0 & tmpdata$Y_1==0,]; 
  y2dat$A_1 = ifelse(y2dat$L1_1==0,0,1)
  y2dat$A = y2dat$A_1; y2dat$L1 = y2dat$L1_1;
  y2dat$y2pred = predict(yfitog, newdata = y2dat, type="response"); 
 
  y2fitextra = glm(y2pred ~ L1_0 + A_0 + L1_0*A_0, family = binomial(), data = y2dat) ; 
  y2dat = tmpdata; y2dat$A_0 = ifelse(y2dat$L1_0==0,0,1);
  ...
  meany2 = mean(y2dat$y2pred)
  
##################
######time 1######
##################
  y1dat = tmpdata; y1dat$A_0 = ifelse(y1dat$L1_0==0,0,1);
  y1dat$A = y1dat$A_0; y1dat$L1 = y1dat$L1_0;
  y1dat$y1pred = predict(yfitog, newdata = y1dat, type="response"); 
  
  meany1 = mean(y1dat$y1pred)
